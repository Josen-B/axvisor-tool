...
#define MAX_RAMS 4
unsigned long long vm_mem[MAX_vmS][MAX_RAMS][4];
...
int get_vm_ram_index(void *vmx_ipa, int vm_id) {
    for (int i = 0; i < MAX_RAMS; i++) {
        if (vm_mem[vm_id][i][MEM_SIZE] == 0)
            continue;
        ;
        if (vmx_ipa >= vm_mem[vm_id][i][vmX_IPA] &&
            vmx_ipa < vm_mem[vm_id][i][vmX_IPA] +
                            vm_mem[vm_id][i][MEM_SIZE]) {
            return i;
        }
    }
    log_error("can't find vm mem index for vmx ipa %#x", vmx_ipa);
    return -1;
}
...
VirtIODevice *create_virtio_device(VirtioDeviceType dev_type, uint32_t vm_id,
                                   uint64_t base_addr, uint64_t len,
                                   uint32_t irq_id, void *arg0, void *arg1) {
    log_info(
        "create virtio device type %s, vm id %d, base addr %lx, len %lx, "
        "irq id %d",
        virtio_device_type_to_string(dev_type), vm_id, base_addr, len,
        irq_id);
...
void init_virtio_queue(VirtIODevice *vdev, VirtioDeviceType type) {
    ...
    log_info("Initializing virtio queue for vm:%d, device type:%s",
             vdev->vm_id, virtio_device_type_to_string(type));
...
void *get_virt_addr(void *vmx_ipa, int vm_id) {
    int ram_idx = get_vm_ram_index(vmx_ipa, vm_id);
    return vm_mem[vm_id][ram_idx][VIRT_ADDR] -
           vm_mem[vm_id][ram_idx][vmX_IPA] + vmx_ipa;
}
...
void virtqueue_set_desc_table(VirtQueue *vq) {
    int vm_id = vq->dev->vm_id;
    log_debug("vm %d set dev %s desc table ipa at %#x", vm_id,
              virtio_device_type_to_string(vq->dev->type), vq->desc_table_addr);
    vq->desc_table = (VirtqDesc *)get_virt_addr(vq->desc_table_addr, vm_id);
}

void virtqueue_set_avail(VirtQueue *vq) {
    int vm_id = vq->dev->vm_id;
    log_debug("vm %d set dev %s avail ring ipa at %#x", vm_id,
              virtio_device_type_to_string(vq->dev->type), vq->avail_addr);
    vq->avail_ring = (VirtqAvail *)get_virt_addr(vq->avail_addr, vm_id);
}

void virtqueue_set_used(VirtQueue *vq) {
    int vm_id = vq->dev->vm_id;
    log_debug("vm %d set dev %s used ring ipa at %#x", vm_id,
              virtio_device_type_to_string(vq->dev->type), vq->used_addr);
    vq->used_ring = (VirtqUsed *)get_virt_addr(vq->used_addr, vm_id);
}

// record one descriptor to iov.
inline int descriptor2iov(int i, volatile VirtqDesc *vd, struct iovec *iov,
                          uint16_t *flags, int vm_id, bool copy_flags) {
    void *host_addr;

    host_addr = get_virt_addr((void *)vd->addr, vm_id);
...
uint64_t virtio_mmio_read(VirtIODevice *vdev, uint64_t offset, unsigned size) {
    ...
    case VIRTIO_MMIO_INTERRUPT_STATUS:
        ...
        log_warn(
            "clear lvz gintc irq injection bit to avoid endless interrupt...");
        ioctl(ko_fd, axvisor_CLEAR_INJECT_IRQ);
#endif
...
void virtio_mmio_write(VirtIODevice *vdev, uint64_t offset, uint64_t value,
    ...
    case VIRTIO_MMIO_DEVICE_FEATURES_SEL:
        ...
        break;
    case VIRTIO_MMIO_DRIVER_FEATURES:
        log_debug("vm %d driver set device %s, accepted features %d",
                  vdev->vm_id, virtio_device_type_to_string(vdev->type),
                  value);
        ...
        if (regs->drv_feature & (1ULL << VIRTIO_RING_F_EVENT_IDX)) {
            log_debug("vm %d driver accepted VIRTIO_RING_F_EVENT_IDX",
                      vdev->vm_id);
    ...
    case VIRTIO_MMIO_DRIVER_FEATURES_SEL:
        ...
        break;
    case VIRTIO_MMIO_QUEUE_SEL:
        log_debug("vm %d driver set device %s, selecting queue %d",
                  vdev->vm_id, virtio_device_type_to_string(vdev->type),
                  value);
        ...
        break;
    case VIRTIO_MMIO_QUEUE_NUM:
        log_debug("vm %d driver set device %s, use virtqueue num %d",
                  vdev->vm_id, virtio_device_type_to_string(vdev->type),
                  value);
    ...
    case VIRTIO_MMIO_QUEUE_READY:
        ...
        break;
    case VIRTIO_MMIO_QUEUE_NOTIFY:
        log_debug("****** vm %d %s queue notify begin ******", vdev->vm_id,
        ...
        log_debug("****** vm %d %s queue notify end ******", vdev->vm_id,
                  virtio_device_type_to_string(vdev->type));
...
// Inject irq_id to target vm. It will add to res list, and notify hypervisor
// through ioctl.
void virtio_inject_irq(VirtQueue *vq) {
    ...
    ioctl(ko_fd, axvisor_FINISH_REQ);
}
...
int virtio_handle_req(volatile struct device_req *req) {
    ...
    // Check if the request corresponds to a virtio device in a specific vm
    for (i = 0; i < vdevs_num; ++i) {
        if ((req->src_vm == vdevs[i]->vm_id) &&
    ...
    if (i == vdevs_num) {
        log_warn("no matched virtio dev in vm %d, address is 0x%x",
                 req->src_vm, req->address);
    ...
    log_trace("src_vm is %d, src_cpu is %lld", req->src_vm, req->src_cpu);
    return 0;
}
...
void virtio_close() {
    ...
    for (int i = 0; i < MAX_vmS; i++) {
        for (int j = 0; j < MAX_RAMS; j++)
            if (vm_mem[i][j][MEM_SIZE] != 0) {
                munmap((void *)vm_mem[i][j][VIRT_ADDR],
                       vm_mem[i][j][MEM_SIZE]);
            }
    }
    mutithread_log_exit();
...
int virtio_init() {
    ...
    // Set process name
    prctl(PR_SET_NAME, "axvisor-virtio", 0, 0, 0);

    // Initialize logging
    multithread_log_init();
    initialize_log();

    log_info("axvisor init");
    ko_fd = open("/dev/axvisor", O_RDWR);
    if (ko_fd < 0) {
        log_error("open axvisor failed");
        exit(1);
    }
    // ioctl for init virtio
    // Communicate with axvisor kernel module
    err = ioctl(ko_fd, axvisor_INIT_VIRTIO);
    if (err) {
...
int create_virtio_device_from_json(cJSON *device_json, int vm_id) {
...
// virtio-gpu
    ...
    if (!create_virtio_device(dev_type, vm_id, base_addr, len, irq_id, arg0,
                              arg1)) {
        return -1;
    }
...
int virtio_start_from_json(char *json_path) {
    char *buffer = NULL;
    u_int64_t file_size;
    int vm_id, num_devices = 0, err = 0, num_vms = 0;
    void *vm0_ipa, *vmx_ipa, *virt_addr;
    ...
    cJSON *vms_json = cJSON_GetObjectItem(root, "vms");
    num_vms = cJSON_GetArraySize(vms_json);
    if (num_vms > MAX_vmS) {
        log_error("Exceed maximum vm number");
        err = -1;
        goto err_out;
    }

    // Match vm information
    for (int i = 0; i < num_vms; i++) {
        cJSON *vm_json = cJSON_GetArrayItem(vms_json, i);
        cJSON *vm_id_json = cJSON_GetObjectItem(vm_json, "id");
        cJSON *memory_region_json =
            cJSON_GetObjectItem(vm_json, "memory_region");
        cJSON *devices_json = cJSON_GetObjectItem(vm_json, "devices");
        vm_id = vm_id_json->valueint;
        if (vm_id >= MAX_vmS) {
            log_error("Exceed maximum vm number");
            err = -1;
            goto err_out;
        ...
        for (int j = 0; j < num_mems; j++) {
            ...
            vm0_ipa = strtoull(
                cJSON_GetObjectItem(mem_region, "vm0_ipa")->valuestring, NULL,
                16);
            vmx_ipa = strtoull(
                cJSON_GetObjectItem(mem_region, "vmx_ipa")->valuestring, NULL,
                16);
            mem_size = strtoull(
            ...
            vm_mem[vm_id][j][VIRT_ADDR] = virt_addr;
            vm_mem[vm_id][j][vm0_IPA] = vm0_ipa;
            vm_mem[vm_id][j][vmX_IPA] = vmx_ipa;
            vm_mem[vm_id][j][MEM_SIZE] = mem_size;
        }

        num_devices = cJSON_GetArraySize(devices_json);
        for (int j = 0; j < num_devices; j++) {
            cJSON *device = cJSON_GetArrayItem(devices_json, j);
            err = create_virtio_device_from_json(device, vm_id);
            if (err) {
