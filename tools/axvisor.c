...
int open_dev() {
    int fd = open("/dev/axvisor", O_RDWR);
    if (fd < 0) {
        perror("open axvisor failed");
        exit(1);
    }
    return fd;
}
...
static int parse_arch_config(cJSON *root, vm_config_t *config) {
...
static int parse_pci_config(cJSON *root, vm_config_t *config) {
...
static int vm_start_from_json(const char *json_config_path,
                                vm_config_t *config) {
    cJSON *root = NULL;
    ...
    cJSON *vm_id_json = cJSON_GetObjectItem(root, "vm_id");
    ...
    CHECK_JSON_NULL_ERR_OUT(vm_id_json, "vm_id")
    ...
    config->vm_id = vm_id_json->valueint;
    ...
    // Iterate through each memory region of the vm
    // Including memory and MMIO regions of the vm
    ...
    if (strlen(name_json->valuestring) > CONFIG_NAME_MAXLEN) {
        fprintf(stderr, "vm name too long: %s\n", name_json->valuestring);
        goto err_out;
    }
    ...
    int fd = open_dev();
    if (fd < 0) {
        perror("vm_start: open axvisor failed");
        goto err_out;
    }

    printf("Calling ioctl to start vm: [%s]\n", config->name);

    int err = ioctl(fd, axvisor_vm_START, config);

    if (err)
        perror("vm_start: ioctl failed");
...
// ./axvisor vm start <path_to_config_file>
static int vm_start(int argc, char *argv[]) {
    int vm_id;
    ...
    return vm_start_from_json(json_config_path, &config);
}

// ./axvisor vm shutdown -id 1
static int vm_shutdown(int argc, char *argv[]) {
    if (argc != 2 || strcmp(argv[0], "-id") != 0) {
        help(1);
    }
    __u64 vm_id;
    sscanf(argv[1], "%llu", &vm_id);
    int fd = open_dev();
    int err = ioctl(fd, axvisor_vm_SHUTDOWN, vm_id);
    if (err)
        perror("vm_shutdown: ioctl failed");
    close(fd);
    return err;
}
...
static int vm_list(int argc, char *argv[]) {
    ...
    __u64 cnt = CONFIG_MAX_vmS;
    vm_info_t *vms = malloc(sizeof(vm_info_t) * cnt);
    vm_list_args_t args = {cnt, vms};
    // printf("vm_list: cnt %llu, vms %p\n", cnt, vms);
    int fd = open_dev();
    int ret = ioctl(fd, axvisor_vm_LIST, &args);
    if (ret < 0)
        perror("vm_list: ioctl failed");
    ...
    //     print_cpu_list(vms[i].cpus, cpu_list_str, sizeof(cpu_list_str));
    //     printf("| %15u | %17s | %15s |\n", vms[i].vm_id, cpu_list_str,
    //            vms[i].name);
    // }
    free(vms);
    close(fd);
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "vm") == 0) {
        if (strcmp(argv[2], "start") == 0) {
            err = vm_start(argc, argv);
        } else if (strcmp(argv[2], "shutdown") == 0) {
            err = vm_shutdown(argc - 3, &argv[3]);
        } else if (strcmp(argv[2], "list") == 0) {
            err = vm_list(argc - 3, &argv[3]);
        } else {
            help(1);
        }
