#ifndef __axvisor_VIRTIO_H
#define __axvisor_VIRTIO_H
#include "cJSON.h"
#include "axvisor.h"
...
struct VirtQueue {
    ...
    uint64_t desc_table_addr; // Descriptor table address (physical address set
                              // by vmx)
    uint64_t
        avail_addr; // Available ring address (physical address set by vmx)
    uint64_t used_addr; // Used ring address (physical address set by vmx)

    // Obtained by get_virt_addr
    volatile VirtqDesc
        *desc_table; // Descriptor table (physical address set by vm0)
    volatile VirtqAvail
        *avail_ring; // Available ring (physical address set by vm0)
    volatile VirtqUsed *used_ring; // Used ring (physical address set by vm0)
    int (*notify_handler)(
        VirtIODevice *vdev,
...
struct VirtIODevice {
    ...
    uint32_t vm_id; // ID of the vm to which it belongs, initialized in
                      // create_virtio_device
    uint32_t irq_id; // Device interrupt ID, initialized in create_virtio_device
    uint64_t base_addr; // The virtio device's base address in non-root vm's
