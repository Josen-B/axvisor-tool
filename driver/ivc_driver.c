...
static int axvisor_ivc_info(void) {
    int err = 0, i;
    if (ivc_info == NULL)
        ivc_info = kmalloc(sizeof(ivc_info_t), GFP_KERNEL);
    err = axvisor_call(axvisor_HC_IVC_INFO, __pa(ivc_info), sizeof(ivc_info_t));
    return err;
}
...
static int axvisor_user_ivc_info(ivc_uinfo_t __user *uinfo) {
    int err = 0, i;
...
static long ivc_ioctl(struct file *file, unsigned int ioctl,
    ...
    int err = 0;
    switch (ioctl) {
    case axvisor_IVC_USER_INFO:
        err = axvisor_user_ivc_info((ivc_uinfo_t __user *)arg);
        break;
    default:
        err = -EINVAL;
        break;
...
// 	node = of_find_node_by_path("/axvisor_ivc_device");
...
static int __init ivc_init(void) {
    ...
    if (!node) {
        // add_ivc_device_node();
        pr_info("axvisor_ivc_device node not found in dtb, can't use ivc\n");
    } else {
        for (i = 0; i < dev_len; i++) {
            soft_irq = of_irq_get(node, i);
            err = request_irq(soft_irq, ivc_irq_handler,
                              IRQF_SHARED | IRQF_TRIGGER_RISING,
                              "axvisor_ivc_device", &ivc_devs[i]);
            if (err) {
