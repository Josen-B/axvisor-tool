...
struct axvisor_event *events[MAX_EVENTS];
static void *epoll_loop() {
    struct epoll_event events[MAX_EVENTS];
    struct axvisor_event *hevent;
    int ret, i;
    for (;;) {
...
struct axvisor_event *add_event(int fd, int epoll_type,
    ...
    hevent = calloc(1, sizeof(struct axvisor_event));
...
int initialize_event_monitor() {
    ...
    if (epoll_fd >= 0)
        ...
        log_error("axvisor_event init failed");
        return -1;
    }
}
