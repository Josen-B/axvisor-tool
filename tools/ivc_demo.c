...
int main(int argc, char *argv[]) {
    ...
    if (is_send) {
        ...
        char *msg = "hello vm1! I'm vm0.";
        strcpy(out, msg);
        tb->ipi_invoke = 1;
        printf("ivc_demo: vm0 sent: %s\n", out);
        ret = poll(&pfd, 1, -1);
        if (pfd.revents & POLLIN)
            printf("ivc_demo: vm0 received: %s\n", in);
        else
            printf("ivc_demo: vm0 poll failed, ret is %d\n", ret);
    } else {
        in = mmap(NULL, tb->out_sec_size, PROT_READ, MAP_SHARED, fd, offset);
        offset += tb->out_sec_size;
        out = mmap(NULL, tb->out_sec_size, PROT_READ | PROT_WRITE, MAP_SHARED,
        ...
        ret = poll(&pfd, 1, -1);
        if (pfd.revents & POLLIN)
            printf("ivc_demo: vm1 received: %s\n", in);
        else
            printf("ivc_demo: vm1 poll failed, ret is %d\n", ret);
        strcpy(out, "I'm vm1. hello vm0! ");
        tb->ipi_invoke = 0;
        printf("ivc_demo: vm1 sent: %s\n", out);
    }
