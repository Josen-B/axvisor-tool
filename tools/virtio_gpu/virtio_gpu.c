...
void virtio_gpu_get_display_info(VirtIODevice *vdev, GPUCommand *gcmd) {
    ...
    for (int i = 0; i < axvisor_VIRTIO_GPU_MAX_SCANOUTS; ++i) {
...
void virtio_gpu_resource_create_2d(VirtIODevice *vdev, GPUCommand *gcmd) {
    ...
    if (res->hostmem + gdev->hostmem >= VIRTIO_GPU_MAX_HOSTMEM) {
        log_error("virtio gpu for vm %d out of hostmem when trying to create "
                  "resource %d",
                  vdev->vm_id, create_2d.resource_id);
        free(res);
        return;
    }
    ...
    log_debug("add a resource %d to gpu dev of vm %d, width: %d height: %d "
              "format: %d mem: %d bytes host-hostmem: %d bytes",
              res->resource_id, vdev->vm_id, res->width, res->height,
...
void virtio_gpu_resource_attach_backing(VirtIODevice *vdev, GPUCommand *gcmd) {
    ...
    log_debug("attaching guest mem to resource %d of gpu dev from vm %d",
              res->resource_id, vdev->vm_id);
...
int virtio_gpu_create_mapping_iov(VirtIODevice *vdev, uint32_t nr_entries,
    ...
    *iov = NULL;
    ...
    for (e = 0, v = 0; e < nr_entries; ++e, ++v) {
        ...
        // Since all memory of vmx will be mapped to vm0
        // And when vm starts, all memory of vmx will be mapped to the
        // virtual memory space of axvisor-tool
        // Therefore, here we only need to manage the virtual address of the
        // memory block used by vmx to store resource data with iov
        ...
        (*iov)[v].iov_base = get_virt_addr((void *)e_addr, vdev->vm_id);
        ...
        // Considering that in future changes, the mapping from vmx to vm0
...
void virtio_gpu_simple_process_cmd(GPUCommand *gcmd, VirtIODevice *vdev) {
    ...
    if (!gcmd->finished) {
        ...
        if (gcmd->error) {
            log_error(
                "failed to handle virtio gpu request from vm %d, and request "
                "type is %d, error type is %d",
                vdev->vm_id, gcmd->control_header.type, gcmd->error);
        }
